...
class Index extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($this->session->userdata('id_pengguna'))){
			...
			if ($rowdata){
				...
				$data['tgl_antrian_layanan']=$rowdata->tgl_antrian_layanan;
				$data['id'] = $rowdata->id;
			}else{
				$data['antrian_pasien']='Pilih Layanan -> Masukkan Tanggal Yang Diinginkan -> Tunggu Nomor Antrian Muncul Sesuai Tanggal yang Dipilih -> Klik Cetak untuk Lihat Kode Antrian.';
				// $data['antrian_pasien']='anda belum mengambil nomor antrian poli';
				$data['nama_layanan']='-';
				$data['tgl_antrian_layanan']='-';
				$data['id'] ="";
			}
    ...
    public function saveAntrian(){
    	$id_layanan = $this->input->post('id_layanan');
		$tanggal = $this->input->post('tgl_antrian_layanan');
